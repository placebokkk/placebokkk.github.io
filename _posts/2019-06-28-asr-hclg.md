...
* pdf-id 决策树绑定后的聚类个数，声学模型中GMM的个数或者神经网络的输出节点个数。

最近我在使用kaldi时，识别错误率(WER)超过了40%，远高于我用的语言模型和声学模型应该达到的错误率。经过一番折腾，终于找到了原因 -- 我没有在lexicon fst(L)里加上自跳转(self-loop)。

在kaldi中，为了使得Grammar fst(G)是determinizable的，G中back-off的边上使用了特殊的'#0'符号(而不是epsilon，但是这样G就是non-determinizable)，因此为了使得compose L和G时可以经过G中输入是'#0'的边，需要在Lexicon fst(L)加上一个自跳转. 因为忘记加这个自跳转边,我的bigram G中back-off边在compose时就被忽略了，使得语言模型缺少了backoff，即解码图里只存在训练集中见过的bigram的路径，从而导致了很高的错误率。而加上self-loop后，不用做其他任何改变，WER就下降到17%。
...
#### step 1
首先，将语言模型中的非法的`<s>`和`</s>`的组合移除,因为这些组合会导致G fst是non-determinizable的(译注：why？)。然后由`arpa2fst`命令将arpa转为binary格式的FST.

请注意，fst中的权重是对概率计算自然对数再取负，而ARPA文件中的数值是对概率计算以10为底的对数。我们来细致的看一下arpa2fst产生的fst。首先这个fst有一个开始节点，表示一句话的开始(node 0),接着有三个节点分别表示`"ache"`, `"Cay"`, `"K."` 这三个词(分别是nodes 6, 4 and 5）;另外，有一个back-off节点(node 1)和一个终止(final)节点(node 2).我们看一下`"<s> ache"`这个bigram词串在fst中的路径，因为在训练语料中没有这个bigram，因此其中会存在back-off路径，`"<s> ache"`对应的路径为0到3，3到1，1到6. 0到3的权重是0;3到1的权重为0.69315，对应了`"<s>"`的back-off值(−ln(10−0.30103));1到6的权重为2.0794，这是`"ache"` (−ln(10−0.9030899))的unigram概率.
